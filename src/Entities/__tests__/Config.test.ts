...
describe('Config', () => {
  test('should create a config', () => {
    const config = createConfig({ matchDuration: 20, resultsPageSize: 10 })
    expect(config).toBeTruthy()
  })

  test('should get config property', () => {
    const config = createConfig({ matchDuration: 20, resultsPageSize: undefined })
    expect(config.matchDuration).toEqual(20)
